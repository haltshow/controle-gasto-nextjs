...
import { deleteOrigemEntrada } from "@/lib/api/actions"

export default async function origemEntrada() {
    const origens: any = await getOrigemEntrada();
    
    return (
        <main className="text-[22px] text-center">
            <div className="mb-10">
                <p className="text-[32px]"> Origem Entrada </p>
            </div>
            <div className="flex justify-start gap-4 mb-4">
                <a href="/origem/entrada/add">
                    ...
                    {origens.length > 0 && origens.map((origem: any) => (
                        <tr className="hover:bg-gray-100 dark:hover:bg-gray-700" key={origem?.id}>
                            ...
                            <td className="py-2 px-3 text-sm font-medium text-center whitespace-nowrap">
                                <a href={`/origem/entrada/edit/${origem.id}`}> <button className="text-blue-600 outline outline-1 rounded-md p-1"> Editar </button> </a>
                            </td>
                            <td className="py-2 px-3 text-sm font-medium text-center whitespace-nowrap">
                                <form action={deleteOrigemEntrada} method="POST">
